...
COPY ./jdk7/jdk-7u80-linux-x64.tar.gz /opt/java/
...
RUN \
 ...
 tar -xf /opt/java/jdk-7u80-linux-x64.tar.gz -C /opt/java/ &&\
 rm -f /opt/java/jdk-7u80-linux-x64.tar.gz

#Instalar glibc para que funcione el InstantClient 11g
LABEL glibc="Instalación glibc libaio libnsl libzip-dev zip"

RUN apk update &&\
 apk add libaio libnsl libzip-dev zip --no-cache &&\
...
#docker-php-ext-install -j$(nproc) oci8 zip &&\
#docker-php-ext-enable oci8 zip
